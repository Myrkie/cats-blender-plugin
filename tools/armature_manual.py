...
class PoseToRest(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Set armature as the active object and ensure it's in pose mode
        bpy.context.view_layer.objects.active = armature_obj
        bpy.ops.object.mode_set(mode='POSE')

        # Now apply the current pose as the new rest pose
    ...
    def apply_armature_to_mesh_with_no_shape_keys(armature_obj, mesh_obj):
        ...
        # In the unlikely case that there was already a modifier with the same name as the new modifier,
        # the new modifier will have ended up with a different name
        mod_name = armature_mod.name

        # Temporarily set the mesh_obj as the active object
        bpy.context.view_layer.objects.active = mesh_obj

        # Make sure we're in object mode before applying the modifier
        if bpy.context.object.mode != 'OBJECT':
            bpy.ops.object.mode_set(mode='OBJECT')

        # Move the modifier to the first index (if Blender version >= 2.90)
        if bpy.app.version >= (2, 90, 0):
            bpy.ops.object.modifier_move_to_index(modifier=mod_name, index=0)
        else:
            # For older versions, move it up manually
